...
async def SyncCommands(self: commands.Bot):
    ...
    try:
        try:
            app_commands.CommandTree.remove_command(self.tree, "promote")
        except Exception:
            pass
        for gid in Multi:
            try:
                existing = await self.tree.fetch_commands(guild=discord.Object(id=gid))
                if any(cmd.name == "promote" for cmd in existing):
                    continue
